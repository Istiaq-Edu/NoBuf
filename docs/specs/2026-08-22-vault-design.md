...
| Different Telegram account logs in | Store `folders` wiped by forceLogout (verified :139-149); SQLite public channels survive → stale vaulted public IDs never render (absent from live list) until pruned on sync via `is_member=false` refresh path |
