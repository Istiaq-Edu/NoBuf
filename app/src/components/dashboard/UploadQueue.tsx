    ...
    return (
        <div className="fixed bottom-4 right-4 w-80 bg-nobuf-surface border border-nobuf-border rounded-xl shadow-2xl overflow-hidden z-[100]">
            <div className="p-3 border-b border-nobuf-border bg-nobuf-hover flex justify-between items-center">
                <h4 className="text-sm font-medium text-nobuf-text">Uploads</h4>
                <div className="flex gap-2">
                    {hasPendingOrActive && (
                        <button onClick={onCancelAll} className="text-xs text-red-400 hover:text-red-300 transition-colors">Cancel All</button>
                    )}
                    <button onClick={onClearFinished} className="text-xs text-nobuf-primary hover:text-nobuf-text transition-colors">Clear Finished</button>
                </div>
            </div>
                    ...
                    <div key={item.id} className="flex flex-col gap-1 p-2 bg-nobuf-hover rounded">
                        <div className="flex items-center gap-3 text-sm">
                            <div className={`w-2 h-2 rounded-full flex-shrink-0 ${item.status === 'pending' ? 'bg-yellow-500' :
                                item.status === 'uploading' ? 'bg-nobuf-ocean-green animate-pulse' :
                            ...
                            {(item.status === 'error' || item.status === 'cancelled') && (
                                <button onClick={() => onRetryItem(item.id)} className="text-gray-400 hover:text-nobuf-ocean-green transition-colors flex-shrink-0" title="Retry">
                            ...
                            <>
                                <div className="w-full bg-nobuf-border h-1 mt-1 rounded-full overflow-hidden">
                                    {item.progress !== undefined ? (
                                        <div
                                            className="bg-nobuf-ocean-green h-full rounded-full transition-all duration-300"
                                            style={{ width: `${item.progress}%` }}
                                        />
                                    ) : (
                                        <div className="bg-nobuf-ocean-green h-full w-full animate-progress-indeterminate" />
                                    )}
                                </div>
